...
use App\Models\SystemResponsibleDocument;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Storage;

class SystemResponsibleDocumentController extends Controller
{
    private const DOC_TYPES = 'resolucion_directoral,resolucion_jefatural,memorando,oficio,contrato,acta,otro';

    public function store(Request $request, System $system, SystemResponsible $responsible)
    {
        ...
        $path = $file->store("system-responsible-docs/{$responsible->id}", 'local');
        ...
        return redirect(route('systems.show', $system) . '#responsibles')->with('success', 'Documento adjuntado correctamente.');
    }

    public function download(System $system, SystemResponsible $responsible, SystemResponsibleDocument $document)
    {
        $disk = $this->disk();
    ...
    public function preview(System $system, SystemResponsible $responsible, SystemResponsibleDocument $document)
    {
        $disk = $this->disk();
    ...
    public function destroy(System $system, SystemResponsible $responsible, SystemResponsibleDocument $document)
    {
        $this->disk()->delete($document->file_path);
        $document->delete();

        return redirect(route('systems.show', $system) . '#responsibles')->with('success', 'Documento eliminado.');
    }
