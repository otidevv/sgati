                    ...
                    <div class="px-5 py-3 group">
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <div class="flex items-center gap-2 text-xs text-gray-400">
                                ...
                                @can('servers.edit')
                                <button onclick="openPortModal({{ $ip->id }}, '{{ $ip->ip_address }}')"
                                        class="opacity-0 group-hover:opacity-100 inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-medium text-blue-500 hover:text-blue-700 hover:bg-blue-50 dark:hover:bg-blue-900/30 transition-all">
                                ...
                                <form method="POST" action="{{ route('admin.servers.ips.destroy', [$server, $ip]) }}"
                                      onsubmit="return confirm('¿Eliminar la IP {{ $ip->ip_address }}? Se eliminarán también sus puertos.')">
                                    @csrf @method('DELETE')
                                    <button type="submit"
                        ...
                        <div class="mt-2 ml-4 flex flex-wrap gap-1.5">
                            @foreach($ip->ports as $p)
                            <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[11px] font-mono
                                         {{ $p->is_active ? 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 border border-indigo-100 dark:border-indigo-800' : 'bg-gray-100 dark:bg-gray-700 text-gray-400 dark:text-gray-500 line-through' }}">
                                :{{ $p->port }}
                                <span class="text-[9px] opacity-50 uppercase">{{ $p->protocol }}</span>
                                @if($p->description)
                                <span class="opacity-60 not-italic text-[10px]" title="{{ $p->description }}">· {{ Str::limit($p->description, 20) }}</span>
                                @endif
                                @can('servers.edit')
                                <form method="POST" action="{{ route('admin.servers.ips.ports.destroy', [$server, $ip, $p]) }}" class="inline">
                                    @csrf @method('DELETE')
                                    <button type="submit" onclick="return confirm('¿Eliminar el puerto {{ $p->port }}?')"
                                            class="ml-0.5 leading-none opacity-50 hover:opacity-100 hover:text-red-500 transition-colors text-sm">×</button>
                                </form>
                                @endcan
                            </span>
                            @endforeach
                    ...
                    <div class="px-5 py-3.5">
                        ...
                        <div class="mt-2 flex flex-wrap items-center gap-1.5 pl-11">
                            ...
                            <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-xs font-mono
                                ...
                                {{ $dep->serverIp->ip_address }}{{ $dep->port ? ':'.$dep->port : '' }}
                            </span>
                            @endif
                            ...
                            <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-xs font-mono
                                ...
                                {{ $eip->ip_address }}{{ ($eip->pivot->port ?? null) ? ':'.($eip->pivot->port) : '' }}
                            </span>
                            @endforeach
                        </div>
                        @endif
        ...
        <form id="form-ip" method="POST" action="{{ route('admin.servers.ips.store', $server) }}"
            ...
            <div class="px-6 py-5 space-y-4">
                ...
                <div>
                    ...
                    <p class="mt-1 text-xs text-gray-400">Los puertos se agregan por separado una vez registrada la IP.</p>
                </div>
                ...
                {{-- Errores de Laravel --}}
                @if($errors->hasAny(['ip_address','type','interface']))
                <div class="px-3 py-2.5 rounded-lg bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 text-sm text-red-700 dark:text-red-400 space-y-0.5">
                    @foreach(['ip_address','type','interface'] as $f)
...
{{-- ── MODAL: Agregar Puerto a IP ──────────────────────────────────── --}}
@can('servers.edit')
<div id="modal-port" class="fixed inset-0 z-50 hidden items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md border border-gray-200 dark:border-gray-700">
        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700">
            <div>
                <h3 class="text-sm font-semibold text-gray-900 dark:text-white">Agregar Puerto</h3>
                <p id="port-modal-ip-label" class="text-xs text-gray-400 font-mono mt-0.5"></p>
            </div>
            <button onclick="closeModal('modal-port')" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors">
                <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                </svg>
            </button>
        </div>
        <form id="form-port" method="POST" action="">
            @csrf
            <div class="px-6 py-5 space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label for="port-number-input" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
                            Puerto <span class="text-red-500">*</span>
                        </label>
                        <input type="number" id="port-number-input" name="port"
                               placeholder="80, 443, 8080…" min="1" max="65535"
                               class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white font-mono text-sm focus:ring-indigo-500 focus:border-indigo-500">
                    </div>
                    <div>
                        <label for="port-protocol-input" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Protocolo</label>
                        <select id="port-protocol-input" name="protocol"
                ...
                {{-- Descripción --}}
                <div>
                    <label for="port-desc-input" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
                        Descripción
                        <span class="font-normal text-xs text-gray-400">(opcional)</span>
                    </label>
                    <input type="text" id="port-desc-input" name="description"
                           placeholder="Ej: HTTP web, API REST, Base de datos…" maxlength="200"
                           class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white text-sm focus:ring-indigo-500 focus:border-indigo-500">
                </div>

                {{-- Errores de Laravel --}}
                @if($errors->hasAny(['port','protocol','description']))
                <div class="px-3 py-2.5 rounded-lg bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 text-sm text-red-700 dark:text-red-400 space-y-0.5">
                    @foreach(['port','protocol','description'] as $f)
                        @error($f)<p>{{ $message }}</p>@enderror
                    @endforeach
                </div>
                @endif
            ...
            <div class="flex items-center justify-end gap-3 px-6 py-4 border-t border-gray-200 dark:border-gray-700">
                <button type="button" onclick="closeModal('modal-port')"
                        class="px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300
                               bg-gray-100 dark:bg-gray-700 rounded-lg hover:bg-gray-200 transition-colors">
                    Cancelar
                </button>
                <button type="submit"
                        class="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium
                               text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 transition-colors shadow-sm">
                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
                    </svg>
                    Agregar Puerto
...
<script>
// ── Modal de IPs ─────────────────────────────────────────────────────
const _existingIps = @json($server->ips->pluck('ip_address'));
...
function validateIpModal() {
    ...
    if (_existingIps.includes(ip)) {
        return showIpError('La IP ' + ip + ' ya está registrada en este servidor.');
    }
    return true;
}
...
// ── Modal de Puertos ─────────────────────────────────────────────────
const _portRoutes = @json($server->ips->mapWithKeys(fn($ip) => [
    $ip->id => route('admin.servers.ips.ports.store', [$server, $ip])
]));

function openPortModal(ipId, ipAddress) {
    document.getElementById('port-modal-ip-label').textContent = ipAddress;
    document.getElementById('form-port').action = _portRoutes[ipId];
    document.getElementById('port-number-input').value = '';
    document.getElementById('port-protocol-input').value = 'tcp';
    document.getElementById('port-desc-input').value = '';
    openModal('modal-port');
    document.getElementById('port-number-input').focus();
}
