    ...
    <div class="flex items-center justify-between h-16 px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex-1 min-w-0 ml-3 lg:ml-0">
            @hasSection('breadcrumb')
                @yield('breadcrumb')
            @else
                <nav class="flex min-w-0" aria-label="Breadcrumb">
                    <ol class="inline-flex items-center space-x-1 md:space-x-2 min-w-0">
                        <li class="inline-flex items-center shrink-0">
                            <a href="{{ route('dashboard') }}" class="inline-flex items-center text-sm font-medium text-gray-700 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400">
                                ...
                                <span class="hidden sm:inline ml-1.5">Inicio</span>
                            </a>
                        </li>
                        @if(View::hasSection('title'))
                        <li class="min-w-0">
                            <div class="flex items-center min-w-0">
                                <svg class="w-5 h-5 shrink-0 text-gray-400 dark:text-gray-500" fill="currentColor" viewBox="0 0 20 20">
                                    <path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd" />
                                </svg>
                                <span class="ml-1 text-sm font-medium text-gray-500 dark:text-gray-400 truncate max-w-[120px] sm:max-w-xs md:max-w-sm lg:max-w-none">@yield('title')</span>
                            </div>
                        </li>
                        @endif
                    </ol>
        ...
        {{-- Right side actions --}}
        <div class="ml-2 sm:ml-4 flex items-center gap-1.5 sm:gap-3">
            {{-- Dark mode toggle --}}
