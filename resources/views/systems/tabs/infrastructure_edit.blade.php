...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Infraestructura</h1>
    ...
    <form action="{{ route('systems.infrastructure.update', $system) }}" method="POST" class="space-y-5">
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100 bg-gray-50/50">
                <h2 class="text-sm font-semibold text-gray-700">Datos del Servidor</h2>
            </div>
            <div class="p-6 space-y-4">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label for="server_name" class="block text-sm font-medium text-gray-700 mb-1">Nombre del Servidor</label>
                        ...
                        @error('server_name')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        <label for="server_os" class="block text-sm font-medium text-gray-700 mb-1">Sistema Operativo</label>
                        <input type="text" id="server_os" name="server_os" value="{{ old('server_os', $infra->server_os) }}"
                               class="block w-full rounded-lg border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                               placeholder="Ubuntu 22.04 LTS">
                        @error('server_os')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        <label for="server_ip" class="block text-sm font-medium text-gray-700 mb-1">IP Interna</label>
                        ...
                        @error('server_ip')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        <label for="public_ip" class="block text-sm font-medium text-gray-700 mb-1">IP Pública</label>
                        <input type="text" id="public_ip" name="public_ip" value="{{ old('public_ip', $infra->public_ip) }}"
                               class="block w-full rounded-lg border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm font-mono"
                               placeholder="200.X.X.X">
                        @error('public_ip')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                    </div>
                </div>
            </div>
        </div>
        ...
        {{-- Web & SSL --}}
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden" x-data="{ ssl: {{ old('ssl_enabled', $infra->ssl_enabled) ? 'true' : 'false' }} }">
            <div class="px-6 py-4 border-b border-gray-100 bg-gray-50/50">
                <h2 class="text-sm font-semibold text-gray-700">Web & SSL</h2>
            </div>
            <div class="p-6 space-y-4">
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div class="sm:col-span-2">
                        <label for="system_url" class="block text-sm font-medium text-gray-700 mb-1">URL del Sistema</label>
                    ...
                    <div>
                        <label for="web_server" class="block text-sm font-medium text-gray-700 mb-1">Servidor Web</label>
                        <input type="text" id="web_server" name="web_server" value="{{ old('web_server', $infra->web_server) }}"
                               class="block w-full rounded-lg border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                               placeholder="Nginx / Apache">
                        @error('web_server')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                    </div>
                    <div>
                ...
                <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                    <div>
                        <p class="text-sm font-medium text-gray-900">Certificado SSL habilitado</p>
                        <p class="text-xs text-gray-500">El sistema usa HTTPS con SSL/TLS</p>
                    </div>
                ...
                <div x-show="ssl" x-transition>
                    <label for="ssl_expiry" class="block text-sm font-medium text-gray-700 mb-1">Fecha de Vencimiento del Certificado</label>
                    ...
                    @error('ssl_expiry')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100 bg-gray-50/50">
                <h2 class="text-sm font-semibold text-gray-700">Notas Adicionales</h2>
            </div>
            <div class="p-6">
