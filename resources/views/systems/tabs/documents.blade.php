<div class="space-y-5">
    ...
    <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl p-4" x-data="{ open: false }">
        <button @click="open = !open" type="button"
                class="flex items-center gap-2 text-sm font-medium text-blue-700 dark:text-blue-300 hover:text-blue-800 dark:hover:text-blue-200 transition-colors w-full">
        ...
        <div x-show="open" x-transition:enter="transition ease-out duration-200"
            ...
            <form action="{{ route('systems.documents.store', $system) }}" method="POST" enctype="multipart/form-data"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                        <label class="block text-xs font-medium text-blue-800 dark:text-blue-300 mb-1">Título <span class="text-red-500">*</span></label>
                        <input type="text" name="title" required placeholder="Nombre del documento"
                               class="block w-full rounded-lg border-blue-200 dark:border-blue-700 dark:bg-gray-700 dark:text-gray-100 bg-white shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-blue-800 dark:text-blue-300 mb-1">Tipo <span class="text-red-500">*</span></label>
                        <select name="doc_type" required class="block w-full rounded-lg border-blue-200 dark:border-blue-700 dark:bg-gray-700 dark:text-gray-100 bg-white shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm">
                    ...
                    <div>
                        <label class="block text-xs font-medium text-blue-800 dark:text-blue-300 mb-1">N° Documento</label>
                        <input type="text" name="doc_number" placeholder="Ej: OFI-2024-001"
                               class="block w-full rounded-lg border-blue-200 dark:border-blue-700 dark:bg-gray-700 dark:text-gray-100 bg-white shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm">
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-blue-800 dark:text-blue-300 mb-1">Fecha de Emisión</label>
                        <input type="date" name="issue_date"
                               class="block w-full rounded-lg border-blue-200 dark:border-blue-700 dark:bg-gray-700 dark:text-gray-100 bg-white shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm">
                    </div>
                </div>
                <div>
                    <label class="block text-xs font-medium text-blue-800 dark:text-blue-300 mb-1">Archivo <span class="text-red-500">*</span></label>
                    <input type="file" name="file" required accept=".pdf,.doc,.docx,.xls,.xlsx,.png,.jpg,.jpeg"
                           class="block w-full text-sm text-blue-700 dark:text-blue-300 file:mr-3 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-medium file:bg-blue-100 dark:file:bg-blue-800 file:text-blue-700 dark:file:text-blue-200 hover:file:bg-blue-200 dark:hover:file:bg-blue-700 cursor-pointer">
                    <p class="mt-1 text-xs text-blue-600 dark:text-blue-400">PDF, Word, Excel, imágenes — máx. 50 MB</p>
                </div>
    ...
    @endphp
    <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-3.5 flex items-start gap-3 shadow-sm hover:shadow-md dark:hover:shadow-gray-700/50 transition-shadow">
        <div class="flex-shrink-0 w-9 h-9 rounded-lg bg-gray-100 dark:bg-gray-700 flex items-center justify-center text-gray-500 dark:text-gray-400">
        ...
        <div class="flex-1 min-w-0">
            ...
            <div class="flex flex-wrap gap-x-3 gap-y-0.5 mt-0.5 text-xs text-gray-400 dark:text-gray-500">
                <span class="text-blue-600 dark:text-blue-400">{{ $doc->doc_type->label() }}</span>
        ...
        <div class="flex items-center gap-1 flex-shrink-0">
            ...
            <form action="{{ route('systems.documents.destroy', [$system, $doc]) }}" method="POST" class="inline">
                @csrf @method('DELETE')
                <button type="button" x-data @click.prevent="sgDeleteForm($el.closest('form'), '¿Eliminar este documento?')"
