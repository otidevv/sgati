@extends('layouts.app')
@section('title', 'Agregar Infraestructura — ' . $system->name)
...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Agregar Infraestructura</h1>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ $system->name }}</p>
        </div>
    </div>

    <form action="{{ route('systems.infrastructure.store', $system) }}" method="POST" class="space-y-5"
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden"
             x-data="{
                ssl: {{ old('ssl_enabled') ? 'true' : 'false' }},
                sslType: '{{ old('ssl_type', 'custom') }}'
             }">
            <div class="px-6 py-4 border-b border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-700/30">
            ...
            <div class="p-6 space-y-4">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div class="sm:col-span-2">
                        ...
                        <input type="text" id="system_url" name="system_url" value="{{ old('system_url') }}"
                    ...
                    <div>
                        ...
                        <input type="text" id="web_server" name="web_server" value="{{ old('web_server') }}"
                               class="block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                ...
                <div x-show="ssl" x-transition class="space-y-4">
                    ...
                    <div x-show="sslType === 'institutional'" x-transition>
                        ...
                        <select id="ssl_certificate_id" name="ssl_certificate_id"
                            ...
                            <option value="{{ $c->id }}"
                                    {{ old('ssl_certificate_id') == $c->id ? 'selected' : '' }}>
                                {{ $c->name }}
        ...
        <div class="flex items-center justify-end gap-3">
            ...
            <button type="submit"
                ...
                <span x-text="submitting ? 'Registrando…' : 'Registrar Infraestructura'"></span>
            </button>
        </div>
    </form>
</div>
...
function infraServerData() {
    return {
        ...
        serverIpId: '{{ old('server_ip_id') }}',
        exposedRows: @json(old('exposed_rows') ?? [['ip_id' => null, 'port' => null]]),
        ips: [],
        serverOpen: false,
        serverQuery: '',
