...
<div class="max-w-2xl mx-auto space-y-6">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Nueva Versión</h1>
            <p class="mt-1 text-sm text-gray-500">{{ $system->name }}</p>
        </div>
    </div>

    <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        <form action="{{ route('systems.versions.store', $system) }}" method="POST" class="p-6 space-y-5">
            @csrf
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                    <label for="version" class="block text-sm font-medium text-gray-700 mb-1">Versión <span class="text-red-500">*</span></label>
                ...
                <div>
                    <label for="release_date" class="block text-sm font-medium text-gray-700 mb-1">Fecha <span class="text-red-500">*</span></label>
                    <input type="date" id="release_date" name="release_date" value="{{ old('release_date', now()->format('Y-m-d')) }}" required
            ...
            <div>
                <label for="changes" class="block text-sm font-medium text-gray-700 mb-1">Cambios / Notas de la versión</label>
                <textarea id="changes" name="changes" rows="4"
                          class="block w-full rounded-lg border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                          placeholder="Describe los cambios, correcciones y nuevas funcionalidades…">{{ old('changes') }}</textarea>
            </div>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label for="git_branch" class="block text-sm font-medium text-gray-700 mb-1">Branch</label>
                    <input type="text" id="git_branch" name="git_branch" value="{{ old('git_branch') }}"
                           class="block w-full rounded-lg border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm font-mono"
                           placeholder="main / release/1.0">
                </div>
                <div>
                    <label for="git_commit" class="block text-sm font-medium text-gray-700 mb-1">Commit Hash</label>
                    <input type="text" id="git_commit" name="git_commit" value="{{ old('git_commit') }}"
                           class="block w-full rounded-lg border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm font-mono"
                           placeholder="abc1234…">
                </div>
            </div>
