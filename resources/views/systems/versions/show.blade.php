@extends('layouts.app')
@section('title', 'v' . $version->version . ' — ' . $system->name)

@section('content')
@php
    $activeResponsibles     = $version->responsibles->where('is_active', true);
    $historicalResponsibles = $version->responsibles->where('is_active', false)->sortByDesc('unassigned_at');
    ...
    $envColors = match($version->environment?->value) {
        ...
        'staging'     => 'bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300',
...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-4">
        <a href="{{ route('systems.show', $system) }}?tab=versions"
        ...
        <div class="flex-1 min-w-0">
            <div class="flex items-center gap-2 flex-wrap">
                ...
                <span class="inline-flex items-center px-2.5 py-0.5 rounded-lg text-xs font-semibold {{ $envColors }}">
                    {{ $version->environment->label() }}
                </span>
                @endif
                @if($version->release_date)
                <span class="text-sm text-gray-500 dark:text-gray-400">{{ $version->release_date->format('d/m/Y') }}</span>
                @endif
            </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-100 dark:border-gray-700">
            <h3 class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Detalles de la versión</h3>
        </div>
        <div class="p-6 space-y-4 text-sm">
            @if($version->changes)
            <div>
                <p class="text-xs text-gray-400 dark:text-gray-500 mb-1">Cambios incluidos</p>
                <p class="text-gray-700 dark:text-gray-300 leading-relaxed">{{ $version->changes }}</p>
            </div>
            @endif
            <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
                @if($version->git_branch)
                <div>
                    <p class="text-xs text-gray-400 dark:text-gray-500 mb-0.5">Rama Git</p>
                    <p class="font-mono text-gray-700 dark:text-gray-300">{{ $version->git_branch }}</p>
                </div>
                @endif
                @if($version->git_commit)
                <div>
                    <p class="text-xs text-gray-400 dark:text-gray-500 mb-0.5">Commit</p>
                    <p class="font-mono text-gray-700 dark:text-gray-300">{{ substr($version->git_commit, 0, 8) }}</p>
                </div>
                @endif
                @if($version->deployedBy)
                <div>
                    <p class="text-xs text-gray-400 dark:text-gray-500 mb-0.5">Desplegado por</p>
                    <p class="text-gray-700 dark:text-gray-300">{{ $version->deployedBy->name }}</p>
                </div>
                @endif
            </div>
            @if($version->notes)
            <div>
                ...
                <p class="text-gray-700 dark:text-gray-300 italic">{{ $version->notes }}</p>
            </div>
            @endif
        </div>
    </div>
                    ...
                    <div class="px-5 py-3.5">
                        <div class="flex items-center gap-3">
                            <div class="w-8 h-8 rounded-full bg-blue-100 dark:bg-blue-900/40 flex items-center justify-center
                                        text-xs font-bold text-blue-700 dark:text-blue-300 flex-shrink-0">
                                {{ $initials }}
                            </div>
                            <div class="flex-1 min-w-0">
                            ...
                            <div class="flex items-center gap-1 flex-shrink-0">
                                <button type="button"
                                        onclick="downloadResponsibleActa('{{ route('systems.versions.responsibles.pdf-data', [$system, $version, $resp]) }}', this)"
                            ...
                            <div class="inline-flex items-center gap-1 pl-2 pr-1 py-0.5 rounded-full
                                ...
                                <button type="button"
                                        onclick='openDocPreview({
                                            ...
                                            previewUrl:  "{{ route('systems.versions.responsibles.documents.preview', [$system, $version, $resp, $doc]) }}",
                                            downloadUrl: "{{ route('systems.versions.responsibles.documents.download', [$system, $version, $resp, $doc]) }}",
                                ...
                                <form action="{{ route('systems.versions.responsibles.documents.destroy', [$system, $version, $resp, $doc]) }}"
                ...
                <div id="resp-history" class="hidden divide-y divide-gray-100 dark:divide-gray-700/60 bg-gray-50/50 dark:bg-gray-800/30">
                    ...
                    <div class="px-5 py-3 flex items-center gap-3 opacity-70">
                        ...
                        <div class="flex items-center gap-1 flex-shrink-0">
                            ...
                            <form action="{{ route('systems.versions.responsibles.destroy', [$system, $version, $resp]) }}"
...
<div id="modal-resp" class="hidden fixed inset-0 z-50 items-center justify-center p-4" role="dialog" aria-modal="true">
    ...
    <div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form id="resp-form"
              action="{{ route('systems.versions.responsibles.store', [$system, $version]) }}"
              method="POST">
            @csrf
            ...
            <div class="p-6 space-y-4">
                ...
                <div>
                    ...
                    <div class="relative" id="resp-search-wrap">
                        ...
                        <div id="resp-selected"
                            ...
                            <span id="resp-selected-name" class="flex-1 text-sm font-medium text-blue-700 dark:text-blue-300 truncate"></span>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
                            Rol en la versión <span class="text-red-500">*</span>
                        </label>
                        <select name="role" id="resp-role" required
                            ...
                            <option value="despliegue">Responsable de Despliegue</option>
            ...
            <div class="flex items-center justify-end gap-3 px-6 py-4 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
                ...
                <button id="ver-resp-submit-btn" type="submit"
                    ...
                    <svg id="ver-resp-submit-icon" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
                    </svg>
                    <svg id="ver-resp-submit-spinner" class="hidden w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24">
...
<div id="modal-doc-upload" class="hidden fixed inset-0 z-50 items-center justify-center p-4" role="dialog" aria-modal="true">
            ...
            <div class="p-6 space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="description" id="doc-description"
                           placeholder="Ej: Resolución de aprobación de la versión"
                           class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700
...
<script>
(function () {
    const storeUrl        = "{{ route('systems.versions.responsibles.store', [$system, $version]) }}";
    const updateBase      = "{{ url('systems/' . $system->id . '/versions/' . $version->id . '/responsibles') }}/";
    const docUploadBase   = "{{ url('systems/' . $system->id . '/versions/' . $version->id . '/responsibles') }}/";
    ...
    let verRespSubmitting = false;
    const verRespBtn     = document.getElementById('ver-resp-submit-btn');
    const verRespIcon    = document.getElementById('ver-resp-submit-icon');
    const verRespSpinner = document.getElementById('ver-resp-submit-spinner');
    const verRespLabel   = document.getElementById('resp-submit-label');

    document.getElementById('resp-form').addEventListener('submit', function (e) {
        if (verRespSubmitting) { e.preventDefault(); return; }
        if (!this.checkValidity()) return;
        verRespSubmitting = true;
        verRespBtn.classList.add('pointer-events-none', 'opacity-75');
        verRespIcon.classList.add('hidden');
        verRespSpinner.classList.remove('hidden');
        verRespLabel.textContent = 'Guardando…';
    });

    function resetVerRespBtn() {
        verRespSubmitting = false;
        verRespBtn.classList.remove('pointer-events-none', 'opacity-75');
        verRespIcon.classList.remove('hidden');
        verRespSpinner.classList.add('hidden');
    }
    ...
    document.querySelector('[onclick="openModal(\'modal-resp\')"]')?.addEventListener('click', function () {
        ...
        verRespLabel.textContent = 'Asignar';
    ...
    window.editResponsible = function (id, data) {
        ...
        verRespLabel.textContent = 'Guardar';
        resetVerRespBtn();
        ...
        document.getElementById('resp-role').value        = data.role        ?? 'desarrollador';
