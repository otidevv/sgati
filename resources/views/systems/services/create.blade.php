@extends('layouts.app')
@section('title', 'Nuevo Servicio / API')
...
<div class="max-w-2xl mx-auto space-y-6">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Nuevo Servicio / API</h1>
            <p class="mt-1 text-sm text-gray-500">{{ $system->name }}</p>
        </div>
    </div>
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        <form action="{{ route('systems.services.store', $system) }}" method="POST" class="p-6 space-y-4">
            @csrf
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" id="auth_type" name="auth_type" value="{{ old('auth_type') }}"
                           class="block w-full rounded-lg border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                           placeholder="API Key / OAuth2 / Basic">
                </div>
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="2"
                          class="block w-full rounded-lg border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                          placeholder="Descripción del servicio o API…">{{ old('description') }}</textarea>
            </div>
