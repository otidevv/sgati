@extends('layouts.app')
@section('title', 'Editar Servicio — ' . $service->service_name)
...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Editar Servicio</h1>
            <p class="mt-0.5 text-sm text-gray-500 dark:text-gray-400">{{ $service->service_name }} — {{ $system->name }}</p>
        </div>
    </div>

    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
        <form action="{{ route('systems.services.update', [$system, $service]) }}" method="POST">
            @csrf @method('PUT')
            ...
            <div class="p-6 grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div class="sm:col-span-2">
                    <label for="service_name" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Nombre del Servicio <span class="text-red-500">*</span></label>
                ...
                <div>
                    ...
                    <select id="direction" name="direction" required
                            class="block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                        <option value="consumed" {{ old('direction', $service->direction) === 'consumed' ? 'selected' : '' }}>Consumido (el sistema usa este servicio)</option>
                        <option value="exposed"  {{ old('direction', $service->direction) === 'exposed'  ? 'selected' : '' }}>Expuesto (el sistema ofrece este servicio)</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select id="environment" name="environment" required
                            class="block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                        <option value="production"  {{ old('environment', $service->environment) === 'production'  ? 'selected' : '' }}>Producción</option>
                        <option value="staging"     {{ old('environment', $service->environment) === 'staging'     ? 'selected' : '' }}>Staging</option>
                        <option value="development" {{ old('environment', $service->environment) === 'development' ? 'selected' : '' }}>Desarrollo</option>
                    </select>
                </div>

                <div>
                    <label for="version" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Versión</label>
                    <input type="text" id="version" name="version" value="{{ old('version', $service->version) }}"
                           placeholder="v1, v2…"
                           class="block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                </div>
                ...
                <div>
                    ...
                    <input type="date" id="valid_from" name="valid_from" value="{{ old('valid_from', $service->valid_from?->format('Y-m-d')) }}"
                           class="block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                </div>

                <div>
                    <label for="valid_until" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Vigencia hasta</label>
                    <input type="date" id="valid_until" name="valid_until" value="{{ old('valid_until', $service->valid_until?->format('Y-m-d')) }}"
                           class="block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                </div>
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea id="description" name="description" rows="2"
                              class="block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm resize-none">{{ old('description', $service->description) }}</textarea>
                </div>
            ...
            <div class="p-6 space-y-4">
                <div>
                    ...
                    <div class="flex gap-4">
                        @foreach(['' => 'Sin proveedor', 'internal' => 'Sistema interno', 'external' => 'Entidad externa'] as $val => $label)
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="radio" name="provider_type" value="{{ $val }}"
                                   {{ old('provider_type', $service->provider_type ?? '') === $val ? 'checked' : '' }}
                ...
                <div id="wrap-internal" class="{{ old('provider_type', $service->provider_type) === 'internal' ? '' : 'hidden' }}">
                    ...
                    <select id="provider_system_id" name="provider_system_id"
                        ...
                        <option value="{{ $s->id }}" {{ old('provider_system_id', $service->provider_system_id) == $s->id ? 'selected' : '' }}>
                ...
                <div id="wrap-external" class="{{ old('provider_type', $service->provider_type) === 'external' ? '' : 'hidden' }}">
                    ...
                    <input type="text" id="provider_name" name="provider_name"
                           value="{{ old('provider_name', $service->provider_name) }}"
                           placeholder="RENIEC, SUNAT…"
                           class="block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                </div>
                ...
                <div>
                    ...
                    <div class="relative" id="requester-search-wrap">
                        ...
                        <input type="hidden" name="requested_by_persona_id" id="requester-persona_id"
                               value="{{ old('requested_by_persona_id', $service->requested_by_persona_id) }}">
                        ...
                        <div id="requester-selected"
                             class="{{ $service->requestedBy ? 'flex' : 'hidden' }} mt-2 items-center gap-2 px-3 py-2 rounded-lg bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700/40">
                            <svg class="w-4 h-4 text-blue-500 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/>
                            </svg>
                            <span id="requester-selected-name" class="flex-1 text-sm font-medium text-blue-700 dark:text-blue-300 truncate">
                                @if($service->requestedBy)
                                    {{ $service->requestedBy->apellido_paterno }} {{ $service->requestedBy->apellido_materno }}, {{ $service->requestedBy->nombres }} ({{ $service->requestedBy->dni }})
                                @endif
                            </span>
                            <button type="button" onclick="clearRequester()" class="text-blue-400 hover:text-red-500 transition-colors">
            ...
            <div class="px-6 py-4 border-t border-b border-gray-100 dark:border-gray-700">
                <h3 class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                    Autenticación y credenciales <span class="font-normal normal-case text-gray-400">(dejar vacío para no modificar)</span>
                </h3>
            </div>
            <div class="p-6 grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div class="sm:col-span-2">
                ...
                <div>
                    ...
                    <input type="password" id="api_secret" name="api_secret" autocomplete="new-password"
                           placeholder="{{ $service->api_secret ? '••••••••' : '' }}"
                           class="block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white font-mono shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="date" id="token_expires_at" name="token_expires_at"
                           value="{{ old('token_expires_at', $service->token_expires_at?->format('Y-m-d')) }}"
                           class="block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                </div>
            </div>
