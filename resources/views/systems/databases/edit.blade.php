@extends('layouts.app')
@section('title', 'Editar Base de Datos — ' . $system->name)
...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Editar Base de Datos</h1>
            <p class="mt-0.5 text-sm text-gray-500 dark:text-gray-400 font-mono">{{ $database->db_name }} — {{ $system->name }}</p>
        </div>
    </div>

    {{-- Formulario --}}
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
        <form action="{{ route('systems.databases.update', [$system, $database]) }}" method="POST">
            @csrf @method('PUT')
            ...
            <div class="p-6 grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <input type="text" id="db_name" name="db_name"
                           value="{{ old('db_name', $database->db_name) }}" required
                ...
                <div>
                    ...
                    <input type="text" id="schema_name" name="schema_name"
                           value="{{ old('schema_name', $database->schema_name) }}"
                           placeholder="public"
                           class="block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700
                                  dark:text-white font-mono shadow-sm
                                  focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                ...
                <div>
                    ...
                    <select id="database_server_id" name="database_server_id"
                        ...
                        <option value="{{ $ds->id }}"
                                ...
                                data-host="{{ $ds->host ?? '' }}"
                                {{ old('database_server_id', $database->database_server_id) == $ds->id ? 'selected' : '' }}>
            ...
            <div class="p-6 grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <input type="text" id="db_user" name="db_user"
                           value="{{ old('db_user', $database->db_user) }}"
            ...
            <div class="px-6 pb-6 border-t border-gray-100 dark:border-gray-700 pt-5">
                ...
                <textarea id="notes" name="notes" rows="2"
                          ...
                          class="block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700
                                 dark:text-white shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm resize-none">{{ old('notes', $database->notes) }}</textarea>
            </div>
...
(function () {
    ...
    // Ejecutar al cargar para sincronizar el valor actual
    syncEngine();
})();
</script>
@endpush
