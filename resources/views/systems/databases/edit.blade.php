@extends('layouts.app')
@section('title', 'Editar Base de Datos')
...
<div class="max-w-2xl mx-auto space-y-6">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Editar Base de Datos</h1>
            <p class="mt-1 text-sm text-gray-500 font-mono">{{ $database->db_name }} — {{ $system->name }}</p>
        </div>
    </div>
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        <form action="{{ route('systems.databases.update', [$system, $database]) }}" method="POST" class="p-6 space-y-4">
            @csrf @method('PUT')
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" id="db_name" name="db_name" value="{{ old('db_name', $database->db_name) }}" required
                ...
                <div class="sm:col-span-2">
                    ...
                    <select id="database_server_id" name="database_server_id"
                        ...
                        <option value="{{ $ds->id }}" {{ old('database_server_id', $database->database_server_id) == $ds->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input type="text" id="db_user" name="db_user" value="{{ old('db_user', $database->db_user) }}"
                           class="block w-full rounded-lg border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm font-mono">
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="text" id="schema_name" name="schema_name" value="{{ old('schema_name', $database->schema_name) }}"
                           class="block w-full rounded-lg border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm font-mono">
                </div>
                <div>
                    <label for="responsible" class="block text-sm font-medium text-gray-700 mb-1">Responsable</label>
                    <input type="text" id="responsible" name="responsible" value="{{ old('responsible', $database->responsible) }}"
                           class="block w-full rounded-lg border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                </div>
                <div>
                    ...
                    <select id="environment" name="environment" required
                            class="block w-full rounded-lg border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                        <option value="production"  {{ old('environment', $database->environment?->value) === 'production'  ? 'selected' : '' }}>Producción</option>
                        <option value="staging"     {{ old('environment', $database->environment?->value) === 'staging'     ? 'selected' : '' }}>Staging</option>
                        <option value="development" {{ old('environment', $database->environment?->value) === 'development' ? 'selected' : '' }}>Desarrollo</option>
                    </select>
                </div>
            </div>
            <div>
                ...
                <textarea id="notes" name="notes" rows="2"
                          class="block w-full rounded-lg border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">{{ old('notes', $database->notes) }}</textarea>
            </div>
